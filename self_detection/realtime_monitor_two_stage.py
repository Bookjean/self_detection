...
class RealtimeMonitorTwoStage(Node):
    def __init__(self):
        ...
        self.stage1_model_name = 'unknown'  # Stage1 모델 이름
        self.stage2_model_name = 'unknown'  # Stage2 모델 이름
        ...
        # 파일명: 모델명_날짜_시간_분.txt (모델 로드 후 생성)
        # 일단 임시 파일명으로 생성하고, 모델 로드 후 업데이트
        now = datetime.now()
        date_str = now.strftime('%Y%m%d_%H%M')  # YYYYMMDD_HHMM
        # 모델 로드 후 _init_log_file()에서 실제 파일명으로 업데이트
        ...
        if TORCH_AVAILABLE:
            if self.stage1_model is not None:
                self.get_logger().info(f'✓ Stage1 model loaded: {self.stage1_type} ({self.stage1_model_name})')
            else:
                self.get_logger().warn('✗ Stage1 model NOT loaded')
            if self.stage2_model is not None:
                self.get_logger().info(f'✓ Stage2 model loaded: {self.stage2_type} ({self.stage2_model_name})')
            else:
    ...
    def _init_log_file(self):
        """Initialize log file with model name in filename."""
        # 파일명: 모델명_날짜_시간_분.txt
        stage1_name = self.stage1_model_name.replace(' ', '_').replace('/', '_')
        stage2_name = self.stage2_model_name.replace(' ', '_').replace('/', '_')
        model_name = f"{stage1_name}_{stage2_name}" if stage2_name != 'unknown' else stage1_name
        filename = f'compensated_two_stage_{model_name}_{self.date_str}.txt'
    ...
    def _load_stage1(self, path):
        try:
            ...
            if 'stage1_' in basename:
                ...
                if len(parts) >= 3:
                    self.stage1_model_name = f"{parts[0]}_{parts[2]}" if len(parts) > 2 else f"{parts[0]}_{parts[1]}"
                else:
                    self.stage1_model_name = 'stage1_mlp'
            else:
                self.stage1_model_name = basename.split('_')[0] if '_' in basename else 'stage1_mlp'
            ...
            self.get_logger().info(f'✓ Stage1 model loaded from {os.path.basename(path)} ({self.stage1_model_name})')
        except Exception as e:
    ...
    def _load_stage2(self, path):
        try:
            ...
            # 모델 이름 추출
            basename = os.path.basename(path)
            if 'stage2_' in basename:
                parts = basename.replace('.pth', '').replace('.pt', '').split('_')
                if len(parts) >= 3:
                    self.stage2_model_name = f"{parts[0]}_{parts[2]}" if len(parts) > 2 else f"{parts[0]}_{parts[1]}"
                else:
                    self.stage2_model_name = 'stage2_tcn'
            else:
                self.stage2_model_name = basename.split('_')[0] if '_' in basename else 'stage2_tcn'
            ...
            self.get_logger().info(f'✓ Stage2 model loaded from {os.path.basename(path)} ({self.stage2_model_name})')
        except Exception as e:
